...
/* 最近一拍的偏航角速度（度/秒），已去零偏并应用尺度因子。
 * 正方向与 Heading_GetYaw() 一致（工程约定：右转为正）。
 * 视觉巡道用它做微分阻尼：视觉只有 20~30 Hz，直接差分视觉误差等于
 * 差分一个零阶保持信号，会产生周期性尖峰；陀螺仪是 100 Hz 且低延迟。
 * 陀螺仪离线时返回 0，控制律退化为纯比例。 */
float Heading_GetYawRate(void);
