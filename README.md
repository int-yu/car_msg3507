...
| `K?` / `K<id>?` / `K<id>=<float>` | 运行时读写控制参数（见 3.3.1 参数表）：列表 / 读单个 / 写入（支持小数与负号，写入立即生效） | id `1~16`；越界返回 `ERR K RANGE MIN=<min> MAX=<max>`，格式错误返回 `ERR K FORMAT` | `K1=1.5` |
...
|---:|---|---|---|---|
| 1 | `wkp` | `MOTION_WHEEL_KP` | PWM/(mm/s) | 0~50 |
| 2 | `wki` | `MOTION_WHEEL_KI` | PWM/mm | 0~50 |
| 3 | `wil` | `MOTION_WHEEL_INTEGRAL_LIMIT` | mm | 0~1000 |
| 4 | `wff` | `MOTION_WHEEL_FEEDFORWARD_PWM_PER_MMPS` | PWM/(mm/s) | 0~10 |
| 5 | `wsf` | `MOTION_WHEEL_STATIC_FRICTION_PWM` | PWM | 0~500 |
...
id 一经发布不得重排，新增参数只能在尾部追加。`lkd` 是巡线权重变化率阻尼：默认 0 时巡线行为与纯离散权重差速完全一致，弧线段左右摆动明显时少量增加抑制震荡。`gsc` 由 `E1`→原地转 N 圈→`Y<n>` 标定流程自动写入；`cpm` 建议用网页里程标定向导（记起点→`F1000`→填卷尺实测→自动换算写入）。
...
1. MotionManager 空闲时使用 `U100/U200/...` 记录 OLED 稳态速度，在线调整速度前馈 `wff`、静摩擦 `wsf`；自动运动开始后固件会拒绝 `L/R/U`，蓝牙仍可发送 Mission 的 `C` 信号。
2. 先令 `wki=0`，逐步增加速度 `wkp` 到响应足够快且不持续振荡，再少量加入 `wki` 消除稳态误差（开 `wki` 前先设积分限幅 `wil`）。
...
| `Application/Debug/Param.c/.h` | 运行时调参注册表：K 命令后端，16 个参数的读写、范围校验与 PID apply 钩子 |
...
前 5 个宏是运行时变量 `MotionWheel_Tune*` 的上电默认值，可经 `K1~K5`（wkp/wki/wil/wff/wsf）在线修改；`MAX_COMMAND_PWM` 保持编译期固定。
