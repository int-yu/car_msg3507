...
| 列名 | 对应掩码位 | 内容 |
|---|---|---|
| `ms` | 固定输出 | 自上电以来的系统节拍累计毫秒数 |
| `yaw` | `TELEMETRY_FIELD_YAW` = `0x01` | Z 轴连续累计航向角（度），不做 ±180° 归一化 |
| `sensor` | `TELEMETRY_FIELD_SENSOR` = `0x02` | 五路灰度位图（高 5 位）与按键位图（低 4 位）拼合的十进制数 |
| `distL,distR` | `TELEMETRY_FIELD_DISTANCE` = `0x04` | 左右轮累计路程 mm |
| `speedL,speedR` | `TELEMETRY_FIELD_SPEED` = `0x08` | 左右轮实测速度 mm/s |
| `mode` | `TELEMETRY_FIELD_MODE` = `0x10` | 运动模式文本：`IDLE`/`LINE`/`STRAIGHT`/`TURN`/`ERROR` |
| `k230x,k230y` | `TELEMETRY_FIELD_K230` = `0x20` | 最近一次 K230 TARGET 的 offsetX/offsetY |

掩码 `TELEMETRY_FIELD_ALL = 0x3F` 开启全部字段（1~6 位均置 1）。例：`M7`（掩码 0x07）只输出 `ms,yaw,sensor,distL,distR`。
