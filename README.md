...
| `M<number>` | 设置遥测字段掩码，立即重发表头 | `1~63`，`0` 或 `>63` 返回 `ERR RANGE`；成功时同时回报新频率 `OK M=<mask> G=<hz>` | `M63` |
...
**`G` 命令的上限是动态的。** `Serial1_SendByte()` 是阻塞发送，发送期间主循环完全停止。字段越多行越长，允许的最高频率越低：全字段（掩码 63）时安全上限约 23 Hz，只开 YAW（掩码 1）时可达 92 Hz。发送 `G<n>` 超过当前安全上限时，返回 `ERR RANGE MAX=<上限>` 而不是固定文本，用户可据此调整频率或先用 `M` 精简字段再提速。
...
| `mode` | `TELEMETRY_FIELD_MODE` = `0x10` | 1 | 运动模式文本：`IDLE`/`LINE`/`STRAIGHT`/`TURN`/`ERROR` |
...
掩码 `TELEMETRY_FIELD_ALL = 0x3F` 开启全部字段（1~6 位均置 1）。例：`M7`（掩码 0x07 = yaw+sensor+distance）输出的表头是 `H,ms,yaw,gray,keys,LD,RD`，共 6 列。
...
| 源文件 / 头文件 | 文件职责 |
|---|---|
| `Application/Comms/BluetoothDebug.c/.h` | 解析 `C/L/R/U/O/D/G/M` 命令，保存单槽任务事件，限制自动运动期间的开环电机调试，并通过 `G`/`M` 命令控制遥测频率与字段掩码 |
...
页面分四块：连接与急停、运动与舵机与拍照控制、遥测频率与字段掩码、曲线与原始终端。几个实现上的取舍：

- **频率上限不在前端校验。** 上限随字段数动态变化（见第 3.3 节），网页直接把 `G` 发给固件，被拒时显示固件回报的 `ERR RANGE MAX=<上限>`，避免前端和固件各存一份阈值而失配。
- **舵机滑块节流 100 ms。** 拖动会逐像素触发事件，不节流会瞬间发出几十条命令占满串口，把遥测流挤垮。
- **曲线各自独立归一化。** `yaw` 量程约 ±180 而速度约 ±500，共用纵轴会把 `yaw` 压成一条直线，所以每条曲线按自己窗口内的极值缩放，纵轴没有统一量纲，只看形状与相对变化。
- **`ms` 和 `mode` 不进曲线选择器。** 前者是横轴，画出来是单调直线；后者是文本。两者仍会写进导出的 CSV。
- **重绘按帧合并。** 遥测最高可达 92 Hz，每收一行就重绘会卡死页面，所以只在有新数据时排一个渲染帧；页面切到后台时浏览器自动暂停重绘，数据仍照常累积。
