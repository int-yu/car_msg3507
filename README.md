...
| `M<number>` | 设置遥测字段掩码，立即重发表头 | `1~65535`，`0` 或 `>65535` 返回 `ERR RANGE`；成功时同时回报新频率 `OK M=<mask> G=<hz>` | `M216` |
...
| `X<mask>` | **板载捕获**：按通道掩码 arm，下一条运动命令自动触发，以 100 Hz 无损写入 RAM（期间不发串口） | 掩码见 3.3.2；通道数超 4 或掩码非法返回 `ERR X MASK MAXCH=4`；成功回 `OK X ARM=<mask> CAP=<最大样本数>` | `X7` |
...
### 3.3.0 双通道数据架构：实时流与板载捕获

调试数据走两条独立通道，用途和取舍完全不同。**用错通道是拿不到可信结论的首要原因。**

| | 通道 A：实时流 | 通道 B：板载捕获 |
|---|---|---|
| 命令 | `M` 掩码 + `G` 频率 | `X<mask>` arm / `X0` dump |
| 前缀 | `H,` 表头 + `D,` 数据 | `CH,` 表头 + `C,` 数据 |
| 频率 | 14~92 Hz（随掩码变化） | **固定 100 Hz，与控制环同频** |
| 传输时机 | 边跑边发 | 记录期间**不发**，动作结束后整体 dump |
| 对主循环的影响 | 阻塞发送，占用带宽预算 20% | **记录期间零阻塞** |
| 容量 | 无限（持续流） | 20 KB RAM，4 通道约 10 秒 |
| 用途 | 监视状态、遥控采数据、急停判断 | **调参分析**、阶跃响应指标 |

**为什么必须有通道 B。** 主循环是 100 Hz（10 ms 一拍），而实时流受阻塞发送限制，全字段只能跑到 14 Hz、轮速子集 30 Hz。一次轮速阶跃的上升沿约 100~200 ms，**30 Hz 采样只能取到 3~6 个点，超调峰值大概率落在两个采样点之间**——据此算出的"超调 10%"实际可能是 18% 或 4%，调 P 增益等于蒙眼。通道 B 在记录期间一个字节都不发串口，数据以完整控制环频率落 RAM，车停稳后才 dump（此时阻塞无害），因此能同时做到高采样率和零控制干扰。

这与 ODrive 的板载 oscilloscope、Betaflight 的 Blackbox 是同一范式：实时曲线用于"确认电机有响应"，真正调参必须用高频记录。

网页的调参试验会自动使用通道 B，并在指标卡上标注数据来源；`X`/`Q` 命令不支持时自动回退到通道 A 并明确提示"指标仅供参考"。

### 3.3.2 板载捕获通道掩码（X 命令）

同时最多选 **4 个通道**（`CAPTURE_MAX_CHANNELS`），超出返回 `ERR X MASK MAXCH=4`。通道越少可录时长越长：4 通道 = 20 字节/样本 = 1024 样本 ≈ 10.2 秒 @100 Hz。
...
| `Application/Debug/Capture.c/.h` | 板载高速捕获：20 KB RAM 缓冲，100 Hz 无损记录最多 4 通道约 10 秒；记录期间零串口输出，动作结束后分批 dump |
...
2. 网页自动完成：`G0` 静默 → 发本阶段的**最优掩码**（调左轮只传 TL/LV/PL）→ 用 `Q` 命令问出固件真实频率上限并自动设频 → **arm 板载捕获** → **清空终端与曲线** → 记录参数快照 → 发激励命令；
3. 动作结束后自动 `X0` 取回 **100 Hz 无损捕获数据**，用它计算阶跃指标（上升时间、超调、稳定时间、稳态误差）；
4. 指标卡明确标注数据来源：`板载 100 Hz 捕获` 表示与控制环同频、指标可信；`实时流 xx Hz` 表示峰值可能漏采、仅供参考；
...
- **指标优先用捕获数据，且必须标注来源。** 实时流受串口带宽限制低于 100 Hz 控制环，阶跃峰值会落在采样点之间；用低采样率数据算出的超调值可能偏差数倍，因此界面上必须让用户看见这次结论是基于哪种数据得出的。
